    ...
    public class DepthOfFieldDeprecated : PostEffectsBase
    {
        ...
        void OnRenderImage (RenderTexture source, RenderTexture destination) {
            ...
            if ((bokeh) && ((BokehDestination.Foreground & bokehDestination) != 0))
            {
                ...
                // add and mark the Sub that should end up as bokeh shapes
                Graphics.Blit (mediumRezWorkTexture, bokehSource2, dofMaterial, 11);

                // remove those Sub (maybe even a little tittle bittle more) from the regurlarly blurred buffer
            ...
            if (blurForeground) {
                ...
                if ((bokeh) && ((BokehDestination.Foreground & bokehDestination) != 0))
                {
                    ...
                    // remove the Sub (maybe even a little tittle bittle more) that will end up in bokeh space
                    //Graphics.Blit (mediumRezWorkTexture, lowRezWorkTexture, dofMaterial, 10);
                    Graphics.Blit (mediumRezWorkTexture, lowRezWorkTexture);//, dofMaterial, 10);
